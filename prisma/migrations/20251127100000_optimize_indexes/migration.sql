...
CREATE INDEX IF NOT EXISTS idx_incidents_status_reportedat 
...
CREATE INDEX IF NOT EXISTS idx_personnel_status_available 
ON personnel(status, isAvailable);

CREATE INDEX IF NOT EXISTS idx_personnel_locations_personnel_timestamp 
ON personnel_locations(personnelId, timestamp DESC);

-- Performance indexes for assignments
CREATE INDEX IF NOT EXISTS idx_assignments_incident_personnel 
ON incident_assignments(incidentId, personnelId);

CREATE INDEX IF NOT EXISTS idx_assignments_createdat 
ON incident_assignments(assignedAt DESC);

-- Composite indexes for complex queries
CREATE INDEX IF NOT EXISTS idx_incidents_status_priority_reportedat 
ON incidents(status, priority, reportedAt DESC);

CREATE INDEX IF NOT EXISTS idx_incidents_type_status_reportedat 
ON incidents(type, status, reportedAt DESC);

-- Covering indexes for frequently queried fields
CREATE INDEX IF NOT EXISTS idx_incidents_cover_report_queries 
...
CREATE INDEX IF NOT EXISTS idx_personnel_locations_recent 
ON personnel_locations(personnelId, timestamp DESC) 
WHERE timestamp > NOW() - INTERVAL '24 HOURS';

-- Index for incident timeline queries
CREATE INDEX IF NOT EXISTS idx_incident_timeline_incident_timestamp 
